  ...
  return (
    ...
    >
      {expanded ? (
        ...
        <div className="relative">
          <div className="flex items-center gap-1.5 h-12 pl-1 pr-1.5 rounded-full bg-white/80 dark:bg-slate-900/90 backdrop-blur-xl border border-white/40 dark:border-white/10 shadow-2xl select-none">
            ...
            <div className="flex-1 min-w-0 max-w-[72px]">
              ...
              {currentLyric && (
                <div className="text-[9px] text-indigo-600 dark:text-indigo-300 font-bold truncate">
                  {currentLyric}
                </div>
              )}
            </div>
              ...
              {playlist.map((song, i) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div
                      className={`text-[11px] font-bold truncate ${i === currentIndex ? "text-indigo-600 dark:text-indigo-300" : "text-slate-800 dark:text-slate-200"}`}
                    >
                      {song.title}
                    </div>
                  ...
                  {i === currentIndex && isPlaying && (
                    <div className="flex items-end gap-[1.5px] h-3 shrink-0">
                      {[0, 1, 2].map((j) => (
                        <div
                          key={j}
                          className="w-[2px] bg-indigo-500 rounded-t-sm animate-pulse"
                          style={{
                            height: `${3 + j * 2}px`,
        ...
        <div
          onClick={() => {
            if (!didDrag.current) setExpanded(true);
          }}
          className="w-24 h-24 rounded-full bg-white/50 dark:bg-slate-800/60 backdrop-blur-md border border-white/40 dark:border-white/10 shadow-xl flex items-center justify-center select-none overflow-hidden cursor-pointer hover:shadow-2xl hover:scale-105 active:scale-95 transition-all duration-200"
        >
          <img
            src={currentSong.cover}
            alt=""
            ...
            <div className="absolute inset-0 flex items-center justify-center bg-black/20 rounded-full">
              <div className="flex items-end gap-[2px] h-3">
                {[0, 1, 2].map((j) => (
                  <div
                    key={j}
                    className="w-[3px] bg-white rounded-t-sm animate-pulse"
                    style={{
                      height: `${4 + j * 3}px`,
                      animationDelay: `${j * 150}ms`,
                    }}
